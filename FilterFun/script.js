...
function upload_image() {
  ...
  photo = new SimpleImage(img);
  image = new SimpleImage(img);
  image1= new SimpleImage(img);
  image2 = new SimpleImage(img);
  image3 = new SimpleImage(img);
  image4 = new SimpleImage(img);
  image5 = new SimpleImage(img);
  photo.drawTo(canvas);
}
...
function reset() {
  if (check(photo)) {
  image = new SimpleImage(photo);
  image1= new SimpleImage(photo);
  image2 = new SimpleImage(photo);
  image3 = new SimpleImage(photo);
  image4 = new SimpleImage(photo);
  image5 = new SimpleImage(photo);
  photo.drawTo(canvas);
  } 
}
...
function filterrainbow() {
  let x = (image3.getHeight())/7;
  Math.round(x);
  for (var pixel of image3.values()){
    let y = pixel.getY();
    avg = (pixel.getRed()+ pixel.getBlue()+ pixel.getGreen())/3;
    if ( y <= x) {
      makestripe(pixel,255,0,0);
    }
    if ( x < y && y <= (2 * x)) {
      makestripe(pixel,255,127,0);
    }
    if ( 2*x < y && y <= 3*x) {
      makestripe(pixel,255,255,0);
    }
    if ( 3*x < y  && y <= 4*x) {
      makestripe(pixel,0,255,0);
    }
    if (4*x <y && y <= 5*x ){
      makestripe(pixel,0,0,255);
    }
    if (5*x <y && y<= 6*x){
      makestripe(pixel,46,43,95);
    }
    if (6*x < y && y <= 7* x ) {
      makestripe(pixel,139,0,255);
    }
  }
}

function makestripe (pixel,r,g,b){
 avg = (pixel.getRed()+ pixel.getBlue()+ pixel.getGreen())/3;
  if (avg <128) {
...
function doblur() {
  var new_image = new SimpleImage (image4.getWidth(), image4.getHeight());
  for (var new_pixel of new_image.values()) {
   var r = Math.random();
   var A = new_pixel.getX();
   var B = new_pixel.getY();
   var w = image4.getWidth();
   var h = image4.getHeight();
    if (r < 0.5) {
      ...
      var old_pixel = image4.getPixel(X,Y);
      new_image.setPixel(A,B,old_pixel);
      }
      else{
        var x = - Math.round(Math.random() * 10);
        var y = - Math.round(Math.random() * 10);
      ...
      var old_pixel = image4.getPixel(A,B);
      new_image.setPixel(A,B,old_pixel);
    }
  }
